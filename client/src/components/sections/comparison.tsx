      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="relative overflow-hidden rounded-3xl bg-secondary/50 border border-border p-8 md:p-12 flex flex-col h-[600px]">
          <div className="flex items-center gap-3 mb-6">
            ...
            <h3 className="text-xl font-bold text-foreground">Traditional Operations</h3>
          </div>
          ...
          <div className="flex-grow relative flex items-center justify-center">
            ...
            <div className="relative w-64 h-64">
              {/* Scattered Nodes */}
              <div className="absolute top-0 left-10 p-3 bg-card rounded-lg text-muted-foreground border border-border"><Mail className="w-5 h-5" /></div>
              <div className="absolute top-10 right-0 p-3 bg-card rounded-lg text-muted-foreground border border-border"><FileSpreadsheet className="w-5 h-5" /></div>
              <div className="absolute bottom-10 left-0 p-3 bg-card rounded-lg text-muted-foreground border border-border"><Phone className="w-5 h-5" /></div>
              <div className="absolute bottom-0 right-10 p-3 bg-card rounded-lg text-muted-foreground border border-border"><Database className="w-5 h-5" /></div>
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 p-3 bg-card rounded-lg text-muted-foreground opacity-50 border border-border"><Server className="w-5 h-5" /></div>

              {/* Broken Lines */}
              <svg className="absolute inset-0 w-full h-full pointer-events-none text-muted-foreground/30">
                <path d="M50 40 L180 50" stroke="currentColor" strokeWidth="1" strokeDasharray="4 4" fill="none" />
                <path d="M50 180 L180 180" stroke="currentColor" strokeWidth="1" strokeDasharray="4 4" fill="none" />
                <path d="M130 130 L180 50" stroke="currentColor" strokeWidth="1" strokeDasharray="4 4" fill="none" />
                <path d="M50 40 L50 180" stroke="currentColor" strokeWidth="1" strokeDasharray="4 4" fill="none" />
                {/* Disconnected line */}
                <path d="M180 180 L150 120" stroke="currentColor" strokeWidth="1" strokeDasharray="4 4" fill="none" />
